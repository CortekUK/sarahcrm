...
// Fallback showcase tiles shown when the `curated_experiences` table is
// empty. Drop out automatically once the admin adds real entries via
// Website → Past Highlights.
const fallbackShowcase = [
  ...
  },
  {
    ...
    title: 'Ibiza Retreat',
    description:
      'Elevate your leadership during five immersive days at the luxurious Fincadelica estate in Ibiza. This transformative retreat blends neuroscience, nature and performance to renew energy, deepen presence and sharpen your vision.',
  ...
  },
  {
    ...
    title: 'Estelle Manor',
    description:
      'Indulge in a two-day escape at Estelle Manor, where country elegance meets alpine charm. Enjoy luxurious arrivals, après-ski themed shopping in Fallow House, gourmet dining and world-class entertainment in a celebration of seasonal sophistication.',
  ...
  },
  {
    ...
    description:
      'Join us for an exclusive afternoon of padel, networking and purposeful giving. Connect with business leaders over dynamic matches, world-class entertainment, cocktails and gourmet street food. A vibrant celebration of sport, impact and opportunity.',
...
export default async function PrivateEventServicesPage() {
  ...
  // Private events are stored in the `events` table with event_type =
  // 'curated_luxury' (and optionally 'retreat'). This page lists the
  // upcoming bookable ones, plus historic curated_experiences as the
  // showcase carousel below.
  const now = new Date().toISOString()

  const [{ data: privateEvents }, { data: pastShowcase }, { data: videos }] = await Promise.all([
    supabase
      .from('events')
      .select(
        'id, slug, title, description, cover_image_url, venue_name, venue_city, start_date, event_type, member_price_pence, guest_price_pence, capacity',
      )
      .eq('status', 'published')
  ...
  const displayPrivateEvents = (privateEvents ?? []).map((e) => ({
    id: e.id,
    slug: e.slug,
    title: e.title,
    ...
    guest_price_pence: e.guest_price_pence,
    capacity: e.capacity,
  }))

  const displayShowcase =
    pastShowcase && pastShowcase.length > 0
      ? pastShowcase.map((e) => ({
          id: e.id,
          title: e.title,
          description: e.description,
  ...
  const displayVideos =
    videos && videos.length > 0
      ? videos.map((v) => ({ id: v.id, title: v.title, youtube_url: v.youtube_url }))
      : fallbackVideos

  return (
    <PrivateEventsContent
      privateEvents={displayPrivateEvents}
